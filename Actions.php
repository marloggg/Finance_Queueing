...
Class Actions extends DBConnection{
    ...
    function get_queuelive(){
        extract($_POST);
        $qry = $this->query("SELECT * FROM `queue_listlive` where queue_id = '{$qid}' ");
        @$res = $qry->fetchArray();
            $resp['status']='success';
        if($res){
            $resp['queue']=$res['queue'];
    ...
    function save_queuelive(){
        $code = sprintf("%'.05d",1);
        while(true){
            $chk = $this->query("SELECT count(queue_id) `count` FROM `queue_listlive` where queue = '".$code."' and date(date_created) = '".date('Y-m-d')."' ")->fetchArray()['count'];
            if($chk > 0){
                $code = sprintf("%'.05d",abs($code) + 1);
